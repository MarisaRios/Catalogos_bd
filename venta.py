# Ventana de Punto de Venta - Tienda Pitico
...
class VentaFrame(wx.Frame):
    def __init__(self, parent=None):
        super().__init__(parent, title="Punto de Venta", size=(900, 600))
    ...
    def agregar_articulo(self, event):
        ...
        sql = """
        SELECT nombre, precio FROM articulos WHERE codigo_barras = %s
        """
    ...
    def finalizar_venta(self, event):
        ...
        for item in self.articulos_seleccionados:
            sql = "SELECT existencia FROM articulos WHERE codigo_barras = %s"
            cursor.execute(sql, (item["codigo"],))
            existencia = cursor.fetchone()[0]
            if item["cantidad"] > existencia:
                wx.MessageBox(f"No hay suficiente stock para {item['nombre']} ({existencia} disponibles).", "Stock insuficiente", wx.OK | wx.ICON_ERROR)
                return

        # Registrar venta
        try:
            fecha = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            total_venta = sum(item["subtotal"] for item in self.articulos_seleccionados)

            sql_venta = """
            INSERT INTO ventas (fecha, total, id_empleado, telefono)
            VALUES (%s, %s, %s, %s)
            """
            cursor.execute(sql_venta, (fecha, total_venta, id_empleado, telefono))
            id_venta = cursor.lastrowid

            # Registrar detalles de venta e inventario
            for item in self.articulos_seleccionados:
                sql_detalle = """
                INSERT INTO detalles_de_venta (codigo_barras, id_venta, cantidad, precio_unitario, subtotal)
                VALUES (%s, %s, %s, %s, %s)
                """
                valores_detalle = (item["codigo"], id_venta, item["cantidad"], item["precio"], item["subtotal"])
            ...
            conexion.commit()
            wx.MessageBox("Venta realizada exitosamente.", "Éxito", wx.OK | wx.ICON_INFORMATION)
            self.Close()
