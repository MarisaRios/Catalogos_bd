# Tienda Pitico - Artículos
# MARISA RIOS DE PAZ S4A

import wx
from db import conexion, cursor

def crear_articulo(event):
    codigo_barras = codigo_barras_entry.GetValue()
    nombre = nombre_entry.GetValue()
    precio = precio_entry.GetValue()
    existencia = existencia_entry.GetValue()
    unidad = unidad_entry.GetValue()
    descripcion = descripcion_entry.GetValue()
    id_categoria = id_categoria_entry.GetValue()

    try:
        sql = """INSERT INTO articulos (codigo_barras, nombre, precio, existencia, unidad, descripcion, id_categoria) 
                 VALUES (%s, %s, %s, %s, %s, %s, %s)"""
        valores = (codigo_barras, nombre, float(precio), int(existencia), unidad, descripcion, int(id_categoria))
        cursor.execute(sql, valores)
        conexion.commit()
        wx.MessageBox("Artículo creado exitosamente", "Éxito", wx.OK | wx.ICON_INFORMATION)
    except Exception as e:
        wx.MessageBox(f"Error al crear artículo:\n{str(e)}", "Error", wx.OK | wx.ICON_ERROR)

def buscar_articulo(event):
    codigo_barras = codigo_barras_entry.GetValue()

    try:
        sql = "SELECT nombre, precio, existencia, unidad, descripcion, id_categoria FROM articulos WHERE codigo_barras = %s"
        cursor.execute(sql, (codigo_barras,))
        resultado = cursor.fetchone()
        if resultado:
            nombre_entry.SetValue(resultado[0])
            precio_entry.SetValue(str(resultado[1]))
            existencia_entry.SetValue(str(resultado[2]))
            unidad_entry.SetValue(resultado[3] or "")
            descripcion_entry.SetValue(resultado[4] or "")
            id_categoria_entry.SetValue(str(resultado[5]))
        else:
            wx.MessageBox("Artículo no encontrado", "Aviso", wx.OK | wx.ICON_WARNING)
    except Exception as e:
        wx.MessageBox(f"Error al buscar artículo:\n{str(e)}", "Error", wx.OK | wx.ICON_ERROR)

def actualizar_articulo(event):
    codigo_barras = codigo_barras_entry.GetValue()
    nombre = nombre_entry.GetValue()
    precio = precio_entry.GetValue()
    existencia = existencia_entry.GetValue()
    unidad = unidad_entry.GetValue()
    descripcion = descripcion_entry.GetValue()
    id_categoria = id_categoria_entry.GetValue()

    try:
        sql = """UPDATE articulos SET nombre = %s, precio = %s, existencia = %s, unidad = %s, descripcion = %s, 
                 id_categoria = %s WHERE codigo_barras = %s"""
        valores = (nombre, float(precio), int(existencia), unidad, descripcion, int(id_categoria), codigo_barras)
        cursor.execute(sql, valores)
        conexion.commit()
        wx.MessageBox("Artículo actualizado correctamente", "Éxito", wx.OK | wx.ICON_INFORMATION)
    except Exception as e:
        wx.MessageBox(f"Error al actualizar artículo:\n{str(e)}", "Error", wx.OK | wx.ICON_ERROR)

def eliminar_articulo(event):
    codigo_barras = codigo_barras_entry.GetValue()

    try:
        sql = "DELETE FROM articulos WHERE codigo_barras = %s"
        cursor.execute(sql, (codigo_barras,))
        conexion.commit()
        wx.MessageBox("Artículo eliminado", "Éxito", wx.OK | wx.ICON_INFORMATION)
    except Exception as e:
        wx.MessageBox(f"Error al eliminar artículo:\n{str(e)}", "Error", wx.OK | wx.ICON_ERROR)

# Crear la app
app = wx.App()

# Crear ventana articulo
ventana2 = wx.Frame(None, title='Artículos', size=(600, 500))
panel = wx.Panel(ventana2)

# Título
titulo = wx.StaticText(panel, label="Artículos", pos=(240, 30))
fuente_titulo = wx.Font(14, wx.DEFAULT, wx.NORMAL, wx.BOLD)
titulo.SetFont(fuente_titulo)

# Campos del formulario
wx.StaticText(panel, label="Código de Barras:", pos=(50, 80))
codigo_barras_entry = wx.TextCtrl(panel, pos=(200, 80), size=(300, -1))
codigo_barras_entry.SetBackgroundColour("light gray")

wx.StaticText(panel, label="Nombre:", pos=(50, 120))
nombre_entry = wx.TextCtrl(panel, pos=(200, 120), size=(300, -1))
nombre_entry.SetBackgroundColour("light gray")

wx.StaticText(panel, label="Precio:", pos=(50, 160))
precio_entry = wx.TextCtrl(panel, pos=(200, 160), size=(100, -1))
precio_entry.SetBackgroundColour("light gray")

wx.StaticText(panel, label="Existencia:", pos=(50, 200))
existencia_entry = wx.TextCtrl(panel, pos=(200, 200), size=(100, -1))
existencia_entry.SetBackgroundColour("light gray")

wx.StaticText(panel, label="Unidad:", pos=(50, 240))
unidad_entry = wx.TextCtrl(panel, pos=(200, 240), size=(150, -1))
unidad_entry.SetBackgroundColour("light gray")

wx.StaticText(panel, label="Descripción:", pos=(50, 280))
descripcion_entry = wx.TextCtrl(panel, pos=(200, 280), size=(300, -1))
descripcion_entry.SetBackgroundColour("light gray")

wx.StaticText(panel, label="ID Categoría:", pos=(50, 320))
id_categoria_entry = wx.TextCtrl(panel, pos=(200, 320), size=(100, -1))
id_categoria_entry.SetBackgroundColour("light gray")

# Botones
boton_ancho = 100
espaciado = 10
total_botones = 4 * boton_ancho + 3 * espaciado
inicio_x = (600 - total_botones) // 2
y_botones = 380

boton_crear = wx.Button(panel, label="Crear", pos=(inicio_x, y_botones), size=(boton_ancho, 30))
boton_buscar = wx.Button(panel, label="Buscar", pos=(inicio_x + (boton_ancho + espaciado), y_botones), size=(boton_ancho, 30))
boton_actualizar = wx.Button(panel, label="Actualizar", pos=(inicio_x + 2 * (boton_ancho + espaciado), y_botones), size=(boton_ancho, 30))
boton_eliminar = wx.Button(panel, label="Eliminar", pos=(inicio_x + 3 * (boton_ancho + espaciado), y_botones), size=(boton_ancho, 30))

# Conectar botones con funciones
boton_crear.Bind(wx.EVT_BUTTON, crear_articulo)
boton_buscar.Bind(wx.EVT_BUTTON, buscar_articulo)
boton_actualizar.Bind(wx.EVT_BUTTON, actualizar_articulo)
boton_eliminar.Bind(wx.EVT_BUTTON, eliminar_articulo)

ventana2.Show()  # Mostrar ventana articulos
app.MainLoop()
